...
Protected Module StringUtils
	...
	#tag Method, Flags = &h1, CompatibilityFlags = (TargetConsole and (Target32Bit or Target64Bit)) or  (TargetDesktop and (Target32Bit or Target64Bit))
		Protected Function sprintf(src as string, ParamArray data as Variant) As string
		  ...
		  // modified: user str and not format gets non locale
		  ...
		  rex.SearchPattern = "(%)(0|/s|'.)?(-)?(\d*)(\.\d+)?([%bcdeufnNpPosxX])"
		  ...
		  do until match = nil
		    if match.SubExpressionCount = 7 then
		      ...
		      case "e" // scientific notation
		        ...
		        Replacement = Left(Replacement, p)+str(Val(Mid(Replacement, p+1)), "+00")
		      case "u" // unsigned int
		        // replacement = Format(datum, "#")
		        replacement = str(datum, "#")
		      case "f" // signed float
		        if padding = "0" then
		          frmstr = "-"+Repeat("0", width)
		          if datum<0 then frmstr = Left(frmstr, Len(frmstr)-1)
		      ...
		      case "n" // numberformat
		        ...
		        dim i, c as integer
		        c = 0 
		        if precision > 0 then c = precision + 1
		        for i = Replacement.Length-c-2 downto 2 step 3
		          if asc(argtype) = &h4E then 
		            replacement = left(Replacement,i-1)+"'" +mid(Replacement,i)
		          else
		            replacement = left(Replacement,i-1)+" " +mid(Replacement,i)
		          end
		        next
		      ...
		      case "p" // percentage
		        ...
		        // Replacement = Format(datum, frmstr)
		        Replacement = str(datum*100.0, frmstr)
		        if asc(argtype) = &h50 then 
		          replacement = replacement + " %"
		        else
		          replacement = replacement + "%"
		        end
